...
public class HttpsTransportManager implements IotHubTransportManager
{
     ...
     * @throws TransportException is the config is null.
     */
    public HttpsTransportManager(DeviceClientConfig config) throws TransportException
    {
        if(config == null)
        {
            //Codes_SRS_HTTPSTRANSPORTMANAGER_21_002: [If the provided `config` is null, the constructor shall throws TransportException.]
            throw new TransportException(new IllegalArgumentException("config is null"));
        }
     ...
     * @throws TransportException if the IotHub communication failed.
     * @throws TransportException if the provided message is null, or invalid.
     */
    public ResponseMessage send(IotHubTransportMessage message) throws TransportException
    {
        ...
        {
            throw new TransportException(new IllegalArgumentException("method or path is null"));
        }
        ...
        switch (message.getIotHubMethod())
        {
            ...
            case POST:
                ...
                break;
            default:
                //Codes_SRS_HTTPSTRANSPORTMANAGER_21_011: [If the IotHubMethod is not `GET` or `POST`, the send shall throws TransportException.]
                throw new TransportException(new IllegalArgumentException("Unknown IoT Hub type " + message.getIotHubMethod().toString()));
        }
